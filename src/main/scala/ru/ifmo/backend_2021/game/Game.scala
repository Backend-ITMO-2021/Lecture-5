...
import ru.ifmo.backend_2021.utils.SudokuUtils._
...
case class Game(player: Player, puzzle: Puzzle, state: State) {
     ...
     */
    private def printResult(state: State): Unit = {
        this.player match {
            case Human =>
                state match {
                    case UNSOLVABLE => println("Sorry, the puzzle now is unsolvable. You just lost your own puzzle!\n")
                    case SOLVED =>
